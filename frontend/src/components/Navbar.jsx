...
const Navbar = ({ onSelectCategory, onSearch }) => {
  ...
  const handleChange = async (value) => {
    ...
    try {
      const response = await axios.get(
        `http://localhost:8083/api/products/search?keyword=${value}`
      );
      setSearchResults(response.data);
  ...
  //         response = await axios.get(`http://localhost:8083/api/products/search?id=${value}`);
  //       } else {
  //         // Input is not a number, search by keyword
  //         response = await axios.get(`http://localhost:8083/api/products/search?keyword=${value}`);
  //       }
